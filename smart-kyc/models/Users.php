...
//    $result = $this->db->executeQuery($sql);
    ...
    public function sendMail($to,$sub,$content) {
        ...
        $mail->SetFrom($this->config->EMAIL_USERNAME);
        $mail->Subject = $sub;
        $mail->Body = $content;
        $mail->AddAddress($to);
    ...
    on r.".$this->config->COL_userRegistration_unique_id." = u.".$this->config->COL_users_userRegistration_unique_id." 
        ...
        $this->sendMail($this->getEmail(),"Reset Password Request",dbconfig::emailContentResetPassword($name,"http://localhost/smart-kyc/resetPassword.html?token=".$token));
    ...
    public function addEmailVerificationToken() {
        ...
        $this->sendMail($this->getEmail(),"Email Verification",dbconfig::emailContentEmailVerification("http://localhost/smart-kyc/users/verifyEmail.php?token=".$token));
        $response['success'] = true;
        $response['result'] = "An email has been sent to your registered email-id";
        return $response;
